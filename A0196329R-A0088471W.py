...
def get_inverted_graph(graph):
    ...
    Args:
        graph (dict): the Bayesian network
    Returns:
        inverted_graph (dict): the Bayesian network with inverted edges
    """
    inverted_graph = {}
...
def traverse_trails(graph, inverted_graph, X, Y, Z, out):
    ...
    while not q.empty():
        ...
        if from_where == from_child:
            ...
            # Head-tail structure going against the edge direction (going from tail to head)
            for parent in inverted_graph[node]:
                if node not in Z and not visited[parent]:
                    q.put((parent, from_child))
        else:
            # Head-tail structure going with the edge direction (going from head to tail)
            for child in graph[node]:
                if node not in Z and not visited[child]:
                    q.put((child, from_parent))
            # v-structure
